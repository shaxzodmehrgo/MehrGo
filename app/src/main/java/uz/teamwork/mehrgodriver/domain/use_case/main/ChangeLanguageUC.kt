...
class ChangeLanguageUC @Inject constructor(
    private val repository: MainRepository,
    private val gson: Gson
) {
    operator fun invoke(language: String): Flow<Resource<BaseResponse<String>>> = flow {
        emit(Resource.Loading)

        try {
            val response = repository.changeLanguage(language)

...
class CancelWithdrawalUC @Inject constructor(
    private val repository: MainRepository,
    private val gson: Gson
) {

    operator fun invoke(id: Int): Flow<Resource<BaseResponse<Any>>> = flow {
        emit(Resource.Loading)

        try {
            val response = repository.paylovWithdrawalCancel(id)

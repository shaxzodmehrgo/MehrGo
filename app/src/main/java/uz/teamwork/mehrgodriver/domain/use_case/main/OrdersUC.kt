...
class OrdersUC @Inject constructor(
    private val mainRepository: MainRepository,
    private val gson: Gson
) {

    operator fun invoke(orderAddressId: String): Flow<Resource<BaseResponse<List<Order>>>> = flow {
        emit(Resource.Loading)
        try {
            val response = mainRepository.getOrders(orderAddressId)

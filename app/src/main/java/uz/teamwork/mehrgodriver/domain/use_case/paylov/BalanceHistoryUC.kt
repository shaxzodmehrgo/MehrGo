...
class BalanceHistoryUC @Inject constructor(
    ...
    operator fun invoke(
        ...
        from: String? = null,
        to: String? = null
    ): Flow<Resource<BaseResponse<BalanceHistory>>> = flow {
        emit(Resource.Loading)

        try {
            val response = repository.getBalanceHistory(page, perPage, type, reason, from, to)

...
class IntroduceUC @Inject constructor(private val repository: AuthRepository) {

    operator fun invoke(): Flow<Resource<BaseResponse<List<Introduce>>>> = flow {
        emit(Resource.Loading)

        try {
            val response = repository.getIntroduceData()

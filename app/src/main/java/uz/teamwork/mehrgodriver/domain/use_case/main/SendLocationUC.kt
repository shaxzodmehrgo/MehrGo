...
class SendLocationUC @Inject constructor(
    private val repository: MainRepository,
    private val gson: Gson
) {
    operator fun invoke(errors: List<ErrorRequest>): Flow<Resource<BaseResponse<Any>>> = flow {
        emit(Resource.Loading)

        try {
            val response = repository.sendErrors(errors)

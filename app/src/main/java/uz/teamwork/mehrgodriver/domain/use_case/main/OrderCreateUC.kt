...
class OrderCreateUC @Inject constructor(
    private val repository: MainRepository,
    private val gson: Gson
) {
    operator fun invoke(request: OrderCreateRequest): Flow<Resource<BaseResponse<Any>>> = flow {
        emit(Resource.Loading)

        try {
            val response = repository.orderCreate(request)

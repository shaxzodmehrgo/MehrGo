...
class CarBrandsUC @Inject constructor(private val repository: AuthRepository) {

    operator fun invoke(): Flow<Resource<BaseResponse<List<CarBrand>>>> = flow {
        emit(Resource.Loading)

        try {
            val response = repository.getCarBrands()

...
import uz.teamwork.mehrgodriver.databinding.FragmentSignUpVerifyBinding
...
class SignUpVerifyFragment : Fragment() {
    ...
    // For ViewModel
    private val signUpVerifyVM: SignUpVerifyVM by viewModels()
    ...
    // Delivery channel chosen on the sign-up screen — the resend must reuse it (and Telegram
    // gets the shorter cooldown). phone drives the "code sent to <phone>" wording.
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Telegram delivery is instant — a much shorter resend cooldown than the SMS timer.
    ...
    private fun applyChannelUi() {
        ...
        if (telegram) {
            ...
            b.tvMessage.text = if (phoneNumber.isNotBlank()) {
    ...
    // Opens the OTP bot (no start payload) so the driver can read the code / link there.
    ...
    private fun resendVia(targetChannel: String) {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            signUpVerifyVM.signUpResendCode(authKey!!, targetChannel).collect {
                when (it) {
                    is Resource.Loading -> {
                        loadingVisible()
    ...
    private fun sendRequest(verifyCode: Int) {
        val localFcm = FcmTokenManager.getToken()
        Timber.tag("FCM").d("user/confirm: attaching device_token=%s", localFcm)
        lifecycleScope.launch {
            signUpVerifyVM.signUpVerifyCode(authKey!!, verifyCode, localFcm)
                .collect {
                    when (it) {
                        is Resource.Loading -> {
                            loadingVisible()
                        ...
                        is Resource.Success -> {
                            // The confirm request carried the FCM token, so the server is now
                            // authoritative for this device.
                            FcmTokenManager.markSynced()
                            Timber.tag("FCM").d("user/confirm OK, marked synced")
                            findNavController().navigate(
                                SignUpVerifyFragmentDirections.actionSignUpVerifyFragmentToCompleteDriverInfoFragment(
    ...
    private fun setTimerView(current: Int) {
        ...
        binding.tvGetNewCode.text = "${getString(R.string.get_new_code)}  $textMinute : $textSecond"
    }
}
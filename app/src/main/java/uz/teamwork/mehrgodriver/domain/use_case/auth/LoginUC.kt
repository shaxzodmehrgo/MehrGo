...
class LoginUC @Inject constructor(private val repository: AuthRepository, private val gson: Gson) {

    operator fun invoke(phoneNumber: String, password: String): Flow<Resource<BaseResponse<User>>> =
        flow {
            emit(Resource.Loading)

            try {
                val response = repository.login(phoneNumber, password)

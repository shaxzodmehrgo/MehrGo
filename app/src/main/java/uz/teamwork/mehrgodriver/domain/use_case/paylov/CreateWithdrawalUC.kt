...
class CreateWithdrawalUC @Inject constructor(
    ...
    operator fun invoke(
        ...
        amount: Long,
        note: String? = null
    ): Flow<Resource<BaseResponse<WithdrawalRequest>>> = flow {
        emit(Resource.Loading)

        try {
            val response = repository.paylovWithdrawalCreate(cardId, amount, note)

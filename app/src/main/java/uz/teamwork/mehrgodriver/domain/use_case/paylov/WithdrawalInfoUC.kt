...
class WithdrawalInfoUC @Inject constructor(
    private val repository: MainRepository,
    private val gson: Gson
) {

    operator fun invoke(): Flow<Resource<BaseResponse<WithdrawalInfo>>> = flow {
        emit(Resource.Loading)

        try {
            val response = repository.paylovWithdrawalInfo()

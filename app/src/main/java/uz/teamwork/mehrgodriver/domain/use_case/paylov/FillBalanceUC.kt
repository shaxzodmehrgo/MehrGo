package uz.teamwork.mehrgodriver.domain.use_case.paylov
...
class FillBalanceUC @Inject constructor(
    private val repository: MainRepository,
    private val gson: Gson
) {

    operator fun invoke(cardId: String, amount: Long): Flow<Resource<BaseResponse<Any>>> = flow {
        emit(Resource.Loading)

        try {
            val response = repository.paylovFillBalance(cardId, amount)

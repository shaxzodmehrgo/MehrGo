...
class OrderCancelReasonsUC @Inject constructor(
    private val mainRepository: MainRepository,
    private val gson: Gson
) {

    operator fun invoke(): Flow<Resource<BaseResponse<List<OrderCancelReason>>>> = flow {
        emit(Resource.Loading)

        try {
            val response = mainRepository.getOrderCancelReasons()

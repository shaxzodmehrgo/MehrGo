...
@AndroidEntryPoint
class RouteNavControllerService2 : LifecycleService() {
...
//        totalDistance = 0f
    ...
    private fun updateLocation(location: Location) {
        if (mLocation != null) {
            val remainingDistance = Helper.calculateBetweenTwoPoints(
            ...
            steps.let { steps ->
                ...
                if (routeProgress <= steps.lastIndex) {
                    if (steps[routeProgress].maneuver?.type == NavMediaNames.NAV_DESTINATION && !warnedFinish) {
                        if (remainingDistance > 99 && remainingDistance < 640) {
                            ...
                            if (getDistanceName(remainingDistance.toDouble(), language).size > 0)
                                list.addAll(getDistanceName(remainingDistance.toDouble(), language))
                ...
                if (distanceInProgress < 50) {
                    sendSoundToPlayer(remainingDistance, steps, 10)
                } else if (distanceInProgress >= 50 && distanceInProgress < 150) {
                    sendSoundToPlayer(remainingDistance, steps, 20)
                } else {
                    sendSoundToPlayer(remainingDistance, steps, 50)
                }
            }
        }
    }

    private fun sendSoundToPlayer(
        remainingDistance: Float,
        steps: List<DirectionLocations.Route.Leg.Step>,
        x: Int
    ) {
        if (remainingDistance < x) {
            routeProgress++
    ...
    private fun startPathAudios(
        ...
        if (routeProgress < steps.size) {
            ...
            if (routeProgress < steps.size - 2) {
                mLocation = LatLng(
                    steps[routeProgress + 1].maneuver!!.location[1],
                    steps[routeProgress + 1].maneuver!!.location[0]
                )
            }
            ...
            routeUC.invoke(lon1, lat1, lon2, lat2).collect {
                when (it) {
                    ...
                    is Resource.Success -> {
                        ...
                        it.data!!.routes[0].legs[0].steps.forEach { item ->
...
//                        val points = Helper.decode(it.data!!.routes[0].geometry, 5)
                        ...
                        isRunning = true
                        steps = it.data.routes[0].legs[0].steps

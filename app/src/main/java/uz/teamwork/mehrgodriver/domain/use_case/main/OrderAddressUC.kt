...
class OrderAddressUC @Inject constructor(
    private val mainRepository: MainRepository,
    private val gson: Gson
) {
    operator fun invoke(): Flow<Resource<BaseResponse<List<OrderAddress>>>> = flow {
        emit(Resource.Loading)

        try {
            val response = mainRepository.getOrderAddress()

...
class TermsOfUseUC @Inject constructor(private val repository: AuthRepository) {

    operator fun invoke(): Flow<Resource<BaseResponse<TermsOfUse>>> = flow {
        emit(Resource.Loading)

        try {
            val response = repository.termsOfUse()
